...
public class Limitless extends Item implements EventReceiver {
	...
	public void appendHoverText(ItemStack stack, @Nullable Level world, List<Component> components, TooltipFlag isAdvanced) {
		components.clear();
		components.add(Component.literal(I18n.get("item.example.limitless")).withStyle(Style.EMPTY.withBold(true).withColor(0xAAAAFF)));
	...
	public void onInventoryTick(PlayerTickEvent event) {
		...
		for (var entity : entities) {
			...
			if (distance < 5) {
				entity.setDeltaMovement(entity.getDeltaMovement().scale(distance / 10));
			}
		} 		
	}
