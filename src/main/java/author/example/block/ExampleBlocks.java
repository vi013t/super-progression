...
import author.example.block.exploration.OpalOre;
import author.example.block.exploration.OpalOreItem;
import author.example.block.general.TinkerTable;
 ...
 * {@link #register(IEventBus)} method which should be called in the {@link Example main} class's constructor with the given
...
public class ExampleBlocks {
	...
	private static <T extends Block, E extends BlockItem> DeferredBlock<T> blockWithItem(String name, Supplier<T> supplier, Supplier<E> itemSupplier) {
		DeferredBlock<T> registryBlock = BLOCKS.register(name, supplier);
		ExampleItems.ITEMS.register(name, itemSupplier);
		return registryBlock;
	}
}
